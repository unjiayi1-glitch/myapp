...
const NIGHT_FIRST = ["下毒者","间谍","男爵","洗衣女","图书管理员","调查员","厨师","感应者","占卜师","管家","小鬼"];
const NIGHT_OTHER = ["下毒者","僧侣","间谍","红衣女","小鬼","渡鸦看守人","殡仪师","感应者","占卜师","管家"];
...
function HomeScreen({ onStart }) {
  return (
    <div style={{ minHeight:"100vh", background:"radial-gradient(ellipse at 50% 30%,#1e0800,"+BG+" 70%)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32, textAlign:"center" }}>
      ...
      <p style={{ color:DIM, fontSize:12, margin:"0 0 4px", letterSpacing:3 }}>BLOOD ON THE CLOCKTOWER</p>
      <p style={{ color:"#5a4a3a", fontSize:12, margin:"0 0 40px" }}>Trouble Brewing · 5–15 人</p>

      <div style={{ maxWidth:300, width:"100%" }}>
        <GoldBtn onClick={onStart}>⚔️ 说书人：创建新游戏</GoldBtn>
      </div>

      <div style={{ marginTop:32, background:CARD, border:"1px solid "+BDR, borderRadius:12, padding:16, maxWidth:300, width:"100%", textAlign:"left" }}>
        <p style={{ color:GOLD, fontSize:12, fontWeight:700, margin:"0 0 10px" }}>📱 游戏流程</p>
        {["说书人输入玩家名字","选择或自动分配角色","通过 WhatsApp 单独发角色给每人","游戏开始，说书人主持白天/夜晚"].map((s,i) => (
          <div key={i} style={{ display:"flex", gap:8, marginBottom:6, alignItems:"flex-start" }}>
            <span style={{ color:GOLD, fontSize:12, minWidth:18 }}>{i+1}.</span>
            <span style={{ color:DIM, fontSize:12, lineHeight:1.6 }}>{s}</span>
          </div>
        ))}
      </div>
    </div>
...
// ─── ROLE SELECT ──────────────────────────────────────────────────
function RoleSelectScreen({ playerCount, selRoles, setSelRoles, onGo, onBack }) {
  const dist = SETUP_TABLE[playerCount] || SETUP_TABLE[5];
  const hasBaron = selRoles.includes("baron");
  ...
  function autoSelect() {
    ...
    [["demon","dm"],["minion","mn"],["outsider","os"],["townsfolk","tf"]].forEach(function(p) {
      shuffle(ALL_ROLES.filter(r => r.type === p[0])).slice(0, need[p[1]]).forEach(r => res.push(r.id));
    });
    setSelRoles(res);
  }
          ...
          return (
            <div key={type} style={{ marginBottom:20 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {ALL_ROLES.filter(r => r.type === type).map(role => {
...
// ─── GAME SCREEN ─────────────────────────────────────────────────
function GameScreen({ players, setPlayers, onEnd }) {
  ...
  const nightOrder = round === 1 ? NIGHT_FIRST : NIGHT_OTHER;
    ...
    <div style={{ minHeight:"100vh", background:BG, color:TEXT, paddingBottom:80 }}>
      <div style={{ padding:"12px 16px", background:phase==="day"?"#150f00":"#050518", borderBottom:"1px solid "+BDR, position:"sticky", top:0, zIndex:10, display:"flex", alignItems:"center", gap:10 }}>
        <div style={{ flex:1 }}>
          ...
          <div style={{ fontSize:11, color:DIM }}>存活 {alive.length} 人</div>
        </div>
...
export default function App() {
  ...
  const [selRoles, setSelRoles] = useState(ALL_ROLES.map(r => r.id));
  ...
  function assignRoles() {
    ...
    function pick(type, n) {
      return shuffle(ALL_ROLES.filter(r => r.type === type && selRoles.includes(r.id))).slice(0, n);
    }
  ...
  return (
    <div style={{ fontFamily:"'Noto Serif SC',Georgia,serif" }}>
      ...
      {screen === "roleselect" && <RoleSelectScreen playerCount={validNames.length} selRoles={selRoles} setSelRoles={setSelRoles} onGo={assignRoles} onBack={() => setScreen("setup")} />}
      {screen === "send"       && <SendRolesScreen players={players} onStart={() => setScreen("game")} />}
      {screen === "game"       && <GameScreen players={players} setPlayers={setPlayers} onEnd={() => setScreen("home")} />}
    </div>
  );
}
